...
token the way Storage Management's JwtAuthFilter does. Two stub-only helpers seed it:
`POST /dev/papers` and `POST /dev/reset`. It is a development aid and should never be
pointed at from a deployed environment.
